// src/app/certificates/error.tsx
"use client";
...
export default function Error({
  ...
  return (
    <div className="min-h-[60vh] flex flex-col items-center justify-center page-offset text-center px-6">
      ...
      <p className="text-[var(--color-text-secondary)] mb-8 max-w-xs text-sm leading-relaxed">
        Could not load this page. This might be a temporary issue.
      </p>
      <div className="flex items-center gap-4">
        <button
          onClick={reset}
          className="inline-flex items-center gap-2 bg-[var(--color-accent)] text-[#F4F1EC] px-5 py-2.5 text-sm font-bold hover:bg-[var(--color-accent-hover)] transition-colors"
          style={{ fontFamily: "var(--font-display)", borderRadius: "var(--radius-sm)" }}
        >
          Try again
        </button>
        <Link href="/" className="text-label text-[var(--color-text-secondary)] hover:text-[var(--color-accent)] transition-colors"
          style={{ fontFamily: "var(--font-mono)" }}>
          ← Home
        </Link>
      </div>

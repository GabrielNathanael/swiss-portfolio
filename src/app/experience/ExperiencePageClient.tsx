      ...
      <div className="container-grid page-header border-b border-[var(--color-border)]">
        <div className="swiss-grid items-end gap-y-6">
          <div className="col-span-12 md:col-span-8">
            <span className="exp-header-el text-label text-[var(--color-accent)] block mb-3">03 / EXPERIENCE</span>
            <h1
              className="exp-header-el font-bold text-[var(--color-text-primary)] leading-none"
          ...
          <div className="col-span-12 md:col-span-4">
            <p className="exp-header-el text-[var(--color-text-secondary)] leading-relaxed text-sm">
      ...
      <div className="exp-list container-grid section-padding">
        <div className="swiss-grid">
          <div className="col-span-12 md:col-start-2 md:col-span-10 relative">
            <div className="timeline-line absolute left-0 top-6 bottom-6 w-px bg-[var(--color-border)] origin-top hidden md:block" style={{ left: "-1px" }} />
            {experiences.map((exp: Experience, i: number) => (
              <div key={exp.id} className="exp-item group relative md:pl-10 py-10 border-b border-[var(--color-border)]">
                {/* Timeline dot */}
                <div
                  className="absolute left-0 top-[50px] w-2.5 h-2.5 rounded-full border-2 border-[var(--color-border)] bg-[var(--color-bg)] group-hover:border-[var(--color-accent)] group-hover:bg-[var(--color-accent)] transition-colors duration-300 hidden md:block"
                ...
                <div className="swiss-grid gap-y-5">
                  ...
                  <div className="col-span-12 md:col-span-3">
                    <span className="text-label text-[var(--color-text-tertiary)] block mb-2">{String(i + 1).padStart(2, "0")}</span>
                    <span className="text-label text-[var(--color-text-secondary)] block leading-relaxed">
                  ...
                  <div className="col-span-12 md:col-span-9">
                    <h2
                      className="font-bold text-[var(--color-text-primary)] group-hover:text-[var(--color-accent)] transition-colors duration-300 mb-1"
                    ...
                    <p className="text-[var(--color-text-secondary)] leading-relaxed mb-5 text-sm">{exp.description}</p>
                      ...
                      <ul className="space-y-2 mb-5">
                        {exp.responsibilities.map((r, ri) => (
                          <li key={ri} className="flex items-start gap-3">
                            <span className="mt-[7px] w-1 h-1 rounded-full bg-[var(--color-accent)] shrink-0" />
                            <span className="text-[var(--color-text-secondary)] text-sm leading-relaxed">{r}</span>
                          </li>
                        ))}
                      </ul>
                    )}
                      ...
                      <div className="flex flex-wrap gap-3 mt-2">
                        {exp.projectWebsite.map((pw) => (
                          <a
                            ...
                            className="text-label text-[var(--color-accent)] hover:underline"
                          >
                            {pw.title} ↗
                          </a>
                        ))}
